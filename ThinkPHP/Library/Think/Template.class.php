...
class  Template {
    ...
    private function parseTemplateName($templateName){
        ...
        foreach ($array as $templateName){
            ...
            //Amango 新增支持函数 file=":functionname|参数" 数组类型  参数1,参数2,参数3合并成 array(参数1,参数2)
            if(substr($templateName,0,1)==':'){
                $matches      = array();
                $matches      = explode('(', str_replace(')', '', $templateName));
                $templateName = call_user_func(str_replace(':', '', $matches[0]),explode('|', $matches[1]));
            } else {
