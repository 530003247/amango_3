...
class InstallController extends Controller{
	...
	protected function _initialize(){
        if(Storage::has(MODULE_PATH . 'Data/install.lock')){
            echo '您已经成功安装了Amango，请不要重复安装!';die;
        }
	}
	...
	public function step2($db = null, $admin = null){
		if(IS_POST){
			...
			if(!is_array($db) || empty($db[0]) ||  empty($db[1]) || empty($db[2]) || empty($db[3])){
				...
				$DB = array();
				list($DB['DB_TYPE'], $DB['DB_HOST'], $DB['DB_PORT'], $DB['DB_NAME'], $DB['DB_USER'], $DB['DB_PWD']
					 , $DB['DB_PREFIX']) = $db;
				//缓存数据库配置
            ...
            $this->assign('randadmin',$this->getrandstr(9));
            $this->assign('randkey',$this->getrandstr(12));
			 $step = session('step');
			 if($step != 1 && $step != 2){
			 	$this->redirect('step1');
			 }
    ...
    public function getrandstr($length){
    	$pattern = 'abcdefghijklmnopqrstuvwxyz0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ!@#$%^&*()_+-=[]{};:|,.<>?'; //字符池
    	$pattern = str_shuffle($pattern);
	    $key     = '';
	    for($i=0;$i<$length;$i++){
	            $key.=$pattern[mt_rand(0,73)];//生成php随机数
	    }
	        return $key;
    }
	//安装第三步，安装数据表，创建配置文件
