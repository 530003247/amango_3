 ...
 * @param array $param 参数 _ADDONS
 * @author 麦当苗儿 <zuojiazi@vip.qq.com>
 */
function addons_url($url, $param = array(), $is_home=''){
    $url        = strpos($url,'://') ? $url : _ADDONS.'://'.$url;
    $url        = parse_url($url);
    ...
    if(strtolower($is_home)=='home'){
        $host   = empty($is_home) ? '' : '/'.ucfirst($is_home);
        return U($host.'/Addons/'.$addons.'/'.$controller.'/'.$action, $param,'',true);
    } else {
        /* 基础参数 */
        $params = array(
            '_addons'     => $addons,
        ...
        $host   = empty($is_home) ? '' : ucfirst($is_home).'/';
        return U($host.'Addons/execute', $params,'',true);
    }
}
/**
 * 时间戳格式化
...
function get_document_model($id = null, $field = null){
    ...
    /* 获取模型名称 */
    if(empty($list)){
       // $map   = array('status' => 1, 'extend' => 14);
$map['status']  = 1;
$map['extend']  = array('in','1,14');
