	...
	public function pay(){
		...
		if(IS_POST&&count($_SESSION['cart'])>0){
			...
			if($orderid){
				...
				$wxbody    = '订单号:'.$orderid;
			    $wxdingdan = $dingdanhao;
			    $wxprice   = $data['order_sumPrice'];
			}else {
				$this->error('生成订单失败!');
			}
			...
			$wxbody    = '订单号:'.$orders['orderId'];
			$wxdingdan = $orders['orderId'];
			$wxprice   = $orders['order_sumPrice'];
		}
		        $this->book_menu($userinfo);
		        //选择支付方式  货到付款  微信支付
		        if(!empty($wxpay_config['hb_dh_diyong'])){
                    ...
                    $userinfp  = session('P');
                    if(!empty($wxdingdan)&&!empty($wxbody)&&!empty($wxprice)){
	                    $booksinfo = array('out_trade_no'=>$wxdingdan.time(),'body'=>$wxbody,'total_fee'=>'1');
			        	$result = Wxpay::get_prepay_id($userinfp['fromusername'],$booksinfo);
						if(false==$result[0]){
							$wxpay_config['Allow_type'] = 'hdfk';
						} else {
							$this->assign('payparams',json_encode($result[1]));
						}
                    } else {
                    	    $wxpay_config['Allow_type'] = 'hdfk';
                    }               
