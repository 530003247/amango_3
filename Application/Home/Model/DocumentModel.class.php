...
class DocumentModel extends Model{
	...
	protected $_auto = array(
		...
		array('usertype', 'getUsertype', self::MODEL_INSERT, 'callback'),
	 ...
	 * @return array              文档列表
	 */
	public function lists($category, $order = '`level` DESC,`digest` DESC,`id` DESC', $status = 1, $field = true){
	...
	public function checkDocunmentAuth($userid,$usertype,$documentid){
		if(!empty($documentid)){
		    //管理员权限
            if($usertype=='admin'){
            	return true;
            	...
            	//只能删除自己设为回收站的帖子
            	$articleinfo = $this->where(array('id'=>$documentid,'status'=>1))->field('uid,usertype')->find();
            	//是否是自己的帖子
            	if($articleinfo['uid']==$userid&&$articleinfo['usertype']==$usertype){
            		return true;
            	}
            }
		}
    ...
    public function setStatus($id){
        ...
        if(!empty($id)){
	        ...
	        $delstatus = $this->where($map)->save(array('status'=>-1));
	        return $delstatus;
        } else {
            return false;
        }
