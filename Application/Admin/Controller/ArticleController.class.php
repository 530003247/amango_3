...
sql;
    ...
    protected function indexOfArticle($cate_id){
        ...
        // Amamngo 新增 加精排序 digest
        $list = $this->lists($Document,$map,'level DESC,digest DESC,id DESC');
        int_to_string($list);
        if($map['pid']){
            // 获取上级文档
            $article    =   $Document->field('id,title,type')->find($map['pid']);
    ...
    /**
     * TODO Aamango 置顶/加精 top/dig
     */
    public function setExtend(){
        $extends = array('top','dig');
        $type    = I('get.type');
        $cate_id = I('get.cate_id');
        $ids     = array_unique((array)I('post.ids'));
        if(in_array($type, $extends)&&is_array($ids)&&!empty($cate_id)){
            switch ($type) {
                case 'top':
                    //获取该分类下最大排序数值
                    $Docmodel = M('Document');
                    $Maxlevel = $Docmodel->where(array('category_id'=>$cate_id))->max('level');
                    $Docnums  = count($ids);
                    foreach ($ids as $key => $value) {
                        $change  = $Maxlevel+$Docnums-$key;
                        $status  = $Docmodel->where(array('category_id'=>$cate_id,'id'=>$value))->save(array('level'=>$change));
                        $this->ag_cms_credits('top',$cate_id,$value,$status);
                    }
                        $this->success('置顶成功');
                    break;
                case 'dig':
                    ...
                    $map['category_id'] = $cate_id;
                    $map['id']          = array('in',$ids);
                    $idstatus = $Docmodel->where($map)->field('id,uid,digest,usertype')->select();
                    $statustitle = '加精';
                    foreach ($idstatus as $key => $value) {
                        empty($value['digest']) && $value['digest']=0;
                        ($value['digest']==1) && $statustitle='取消加精';
                        $result = ($value['digest']%100)+1;
                        $status = $Docmodel->where(array('category_id'=>$cate_id,'id'=>$value['id']))->save(array('digest'=>$result));
                        $this->ag_cms_credits('dig',$cate_id,$value,$status);
                    }
                        $this->success($statustitle.'成功');
                    break;
                default:
                        $this->error('不支持该类型操作');
                    break;
