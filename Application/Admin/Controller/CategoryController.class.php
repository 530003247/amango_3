...
class CategoryController extends AdminController {
        ...
        if(IS_POST){
            ...
            if(false !== $Category->update()){
                ...
                if($replytype!=$oldreply){
                    //如果原来有 则删除表
                    if($oldreply==1){
                        $delstatus = self::delAutoreply($replyname);
                        if(!$delstatus){
            ...
            $tpllist = api('System/getCategoryThemelist');
    ...
    //删除自动回复列表
    protected function delAutoreply($replyname){
        $replytablename  = D('Replydocument')->replytable;
        $model    = M('Model');
        $model_id = $model->where(array('name'=>$replyname))->getField('id');
        if(!empty($model_id)){
            $table_name = C('DB_PREFIX').$replytablename.'_'.$replyname;
            //删除属性数据
            M('Attribute')->where(array('model_id'=>$model_id))->delete();
            //删除模型数据
            $model->where(array('id'=>$model_id))->delete();
        //删除该表
        $sql = <<<sql
...
sql;
    ...
    protected function setAutoreply($model_id,$replyname,$replytitle){
        ...
        //获取该分类回复模型id
        $modelid  = api('Model/get_model_id',array('name'=>$replyname));
        if(empty($modelid)){
            //获取基础回复模型 继承基础回复模型
            $replytablename  = D('Replydocument')->replytable;
