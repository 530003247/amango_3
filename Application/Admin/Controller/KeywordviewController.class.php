...
class KeywordviewController extends AdminController {
    ...
    public function addkeyword(){
        if(IS_AJAX){
            ...
            $docList = $this->indexLink();
            $this->assign('indexList',$docList['index']);
            $this->assign('listList',$docList['list']);
        ...
        foreach ($tree as $key => $value) {
            if(is_array($value['_'])){
                foreach ($value['_'] as $k => $v) {
                    $indexList[U('/Articles/'.$v['id'],'','',true)]  = $v['title'];
                    $listList[U('/Articles/'.$v['name'],'','',true)] = $v['title'];
                }
            } else {
                    $indexList[U('/Articles/'.$value['id'],'','',true)]  = $value['title'];
                    $listList[U('/Articles/'.$value['name'],'','',true)] = $value['title'];
            }
        }

        return array('index'=>$indexList,'list'=>$listList);
    }
